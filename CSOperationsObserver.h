...
- (void)operationsObserver:(CSOperationsObserver*)observer
    didStartObservingOperationItem:(CSOperationObserverItem*)item
 ...
 *  @warning CSOperationsObserver is intended to observe an operation only once,
 *           do not observe an operation twice or try to observe it
 *           with a different identifier.
 *
 * @return a reference to the then created observer item or nil
 ...
 *  @return a reference to the then created observer item
 *          or nil if the operation could not be created.
 */
- (CSOperationObserverItem*)observeOperation:(NSOperation*)operation
                                  identifier:(NSString*)identifier;

/** Convenience method for regsitering an operation
 *  that is related to an index path
 *  @return a reference to the then created observer item
 *          or nil if the operation could not be created.
 */
- (CSOperationObserverItem*)observeOperation:(NSOperation*)operation
 ...
 *  @return the operation item associated with the identifier
 *         or nil if there is no operation associated with it.
 */
- (CSOperationObserverItem*)operationItemWithIdentifier:(NSString*)identifier;

/** A convenience method to cancel an operation using the identifier.
 *  If there is no operation associated with the identifier,
